...
# 1. 页面配置
st.set_page_config(page_title="Tripeaks 算法对比平台 V1.9.13", layout="wide")
st.title("🎴 Tripeaks 算法对比与深度审计平台 V1.9.13")
...
def audit_engine(row, col_map, base_init_score, burst_window, burst_threshold):
    ...
    if total_eliminated > 0 and len(seq) >= burst_window:
        for i in range(len(seq) - burst_window + 1):
            ...
            burst_rate = window_sum / total_eliminated
            if burst_rate > max_burst_rate:
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("⚠️ 节奏风控参数")
    burst_win = st.number_input("连续手牌数 (窗口大小)", min_value=1, max_value=10, value=3)
    ...
    if all_raw_dfs:
        ...
        with st.spinner('节奏风控数据处理中...'):
            ...
            for (f_name, h_val, j_id, d_val), gp in df.groupby(['__ORIGIN__', h_col, j_col, d_col]):
                ...
                reason = "✅ 通过"
                if red_m.mean() >= 0.15: 
                    reason = f"❌ 红线拒绝 ({gp[red_m]['红线判定'].mode()[0]})"
        ...
        for h_v, gp_h in df_fact.groupby('初始手牌'):
            ...
            row = {"初始手牌数": h_v, "牌集总数": total_unique_jid, "✅ 总通过数(去重)": total_pass_jid, "覆盖率": total_pass_jid/total_unique_jid if total_unique_jid>0 else 0}
        ...
        ).format({"μ_均值":"{:.2f}", "σ²_方差":"{:.2f}", "CV":"{:.3f}", "max_burst":"{:.1%}"}), use_container_width=True)

        st.info(f"💡 对齐检查：明细表显示 {view_df[view_df['is_pass']==1].shape[0]} 行通过记录。")
